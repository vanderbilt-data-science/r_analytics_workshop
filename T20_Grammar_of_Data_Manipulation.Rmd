...
How many flights left on December 12. 
...
Let's say that we'd like to do the above calculation, and then look at the most-delayed flights, and filter out those with delays less than an hour. We could write the following:
...
**Example 1: What is the average flight distance?**

```{r Example 1}
```

It's often useful to check the column names really quickly. We can do
this by simply calling `colnames()`

```{r Reminder about colnames}
flights_df %>%
    colnames()
```

**Exercise 1: What is the minimum flight duration?** Find it
using the approach in Example 1, but by calling `min()` instead
of `mean()`! WARNING: You will run into a problem. When you do,
put up your purple sticky [1.5 minutes]

```{r Exercise 1}
```

**Example 2: Are there flights that are missing air_time values?**
...
**Example 3: What is the 90th quantile of flight duration?**

```{r Example 3}
```

**Exercise 2: What are the 25th, 50th, and 75th quantiles of flight**
**duration?** Find these using only a single summarize() call! And make
sure to give each value a reasonable variable name. Put up your green
sticky when you're done. [1.5 minutes]

```{r Exercise 2}
```

# Grouped summary functions

**Example 4: How many flights did each carrier make?**

```{r Example 4}
```

**Exercise 3: Within each month, can I get the average distance**
**and median air time?** Use the approach from Example 4. WARNING:
If you have a problem using `median()`, remember the issue that we
ran into with `min()` and how we solved that. Put up your green
sticky when you're done. [3 minutes]

```{r Exercise 3}
```

**Example 5: How many flights were there between each airport**
**pair?** For example, I want to know how many flights there were
from JFK to LAX, how many  flights there were from LaGuardia to
O'Hare, etc.
...
**Exercise 4: Did any of the carriers use the same plane from the**
**same origin to the same destination airport more than once in the same**
**day? If so, how many times for each occurrence?** WARNING: you'll run
into a problem with missing values for one of your variables. You
will need to `filter()` them out at the beginning using a function
that Dr. Spencer-Smith taught you earlier. HINT: To only see the
combinations that occurred multiple times, you will need to perform
a `filter()` at the end to get rid of the combinations that only
occurred once. Put up your green sticky when you're done. [5 minutes]
