...
Let's look at a few cars have a higher miles per gallon than you might expect. 
...
To change the geom in your plot, change the geom function that you add to `ggplot()`. Now let's layer these plots:
...
```    
Note that this does not generate a legend for you.


Implicitly add grouping by assigning a catagorical variable to an aesthetic.
```{r}
ggplot(data = mpg) +
  geom_smooth(
